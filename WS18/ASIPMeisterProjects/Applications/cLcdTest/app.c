...
//int drawRectangle(const int ax, const int ay, const int bx, const int by, const int cx,const int cy, const int dx,const int dy) {
  /*
  printCoordinates(a);
  t_print(" ");
  printCoordinates(b);
...
/*
  g_drawline(ax, ay, bx, by);
  g_drawline(bx, by, cx, cy);
  g_drawline(cx, cy, dx, dy);
  g_drawline(dx, dy, ax, ay);
*/
...
int overwriteRectangle(const struct point* const a, const struct point* const b, const struct point* const c, const struct point* const d) {
  ...
  int args[1];
  args[0] = 2;
  sendcommand('G', 'V', args, 0, 1, 0, LCD_I2C_ADDR);
...
int delete() {
  ...
  sendcommand('R', 'L', args, 0, 8, 0, LCD_I2C_ADDR);
...
int main() {
  ...
  drawRectangle(&a, &b, &c, &d);
  //drawRectangle(80,80,130,80,130,130,80,130);
