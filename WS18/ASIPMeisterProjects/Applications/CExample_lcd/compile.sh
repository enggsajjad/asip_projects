...
${CP} -r ${APPLICATION_NAME}.mem ${ISE_DIR}/.
 ...
 # dlxsim simulate
  ...
  ${SED} '/.glob/s/^/;/; /.align/s/^/;/; /.type/s/^/;/; /.size/s/^/;/; /.ident/s/^/;/; /.zero/s/^/;/; s/.long/.data.32/; /%hi(/ s/)/\/0x10000)/; /%lo(/ s/)/\%0x10000)/; /%hi(/ s/%hi(/(/;/%lo(/ s/%lo(/(/' app.s  > app.1s

  ${MKIMG_DIR}/renameLabels.pl app.1s 'app' > app.2s
  ${MKIMG_DIR}/renameLC.pl     app.2s 'app' > app.3s

  ${ECHO} -e "$(${CAT}  app.3s) \n\n_exit: \n\ttrap #0 \n"  >  app.4s

  ${AWK} -v RS='.data.*.text' -v ORS= '1;NR==1{printf ".text"}' app.4s > app.5s
  ${AWK} '/.data/{flag=1} /.text/{flag=0} flag' app.4s >> app.5s

  ${SED} '/^.data/ s/.data/\t.section\t.data\n\t.addressing\t.Word/; /^.text/ s/.text/\t.section\t.text\n\t.addressing\t.Word/' app.5s > app.6s

  ${SED} s/REPLACE/${STACK_START}/ $MKIMG_DIR/startup_dlx2.s > startup_dlx2.s

  ${SED} "/.section\t.text/ r startup_dlx2.s" app.6s > ${APPLICATION_NAME}.dlxsim
