...
"OP-code"{"binary"{"000000"},width{"11","6"}},
"OP-code"{"binary"{"010010"},width{"5","0"}}
},"ROT"{type{"RI"},"Operand"{"name"{"const"},width{"31","16"}},
"Operand"{"name"{"rs1"},width{"15","11"}},
"Operand"{"name"{"rd"},width{"10","6"}},
...
addr_width{"5"},
stage_number{"2"}}}
,"SFTvar1"{class{"shifter"},classpath{"/basicfhmdb/computational/"},use{"(unspecified)"},
parameter{
abstraction_level{for_simulation{"Behavior"},for_synthesis{"Gate"}},
bit_width{"32"},
amount{"variable"}}}
...
reset_handler = \"00000000000000001111000000000000\"; // 0x0000F800 (fixed)
...
//null                 = [relative_branchz_cond] PC.write(relative_branchz_abs_addr); // branch
null                 = [cmp_ugte] PC.write(relative_branchz_abs_addr); // branch"},
clk(4){""}
}
...
,"ROT"{variable{"wire [31:0]  sft_result;
wire [31:0]  sft_result1;
wire [31:0]  sft_amt_32;
wire [4:0] sft_amt;
wire [4:0] sft_amt1;
wire [31:0]  alu_result;
wire [3:0]   alu_flag;
...
thirtytwo = \"00000000000000000000000000100000\";
zero27 = \"000000000000000000000000000\";
sft_amt_32 = <zero27,sft_amt>;
...
<alu_result, alu_flag> = ALU.sub(thirtytwo, sft_amt_32);
update_streg_flag_reserved_bits = '0';
update_streg_flag_stfl_in       = <update_streg_flag_reserved_bits, alu_flag>;

// we can update flag using the function of flag_update
null                            = GPR.flag_update(update_streg_flag_stfl_in);
sft_amt1 = alu_result[4:0];
// shift execution
sft_result1       = SFTvar1.sll(source1, sft_amt1);

<alu_result1, alu_flag1> = ALU1.or(sft_result, sft_result1);
// foward alu_result1 to as GPR[rd] from stage EXE
null = FWU0.forward1(rd, alu_result1);
null = FWU1.forward1(rd, alu_result1);"},
clk(4){"null = GPR.write0(rd, alu_result1);        // write alu_result1[arg2] to GPR[arg1]
// foward alu_result1 to as GPR[rd] from stage WB
null = FWU0.forward2(rd, alu_result1);
null = FWU1.forward2(rd, alu_result1);"}
}
}}
