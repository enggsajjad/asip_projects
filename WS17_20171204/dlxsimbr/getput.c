...
#ifndef lint
/*static char rcsid[] = "$Header: /home/cvs/cvs_repos/dlxsim/dlx/dlxsim/getput.c,v 1.1.1.2 2004/08/30 09:15:05 sascha Exp $ SPRITE (Berkeley)";*/
#endif
    ...
    char **argv;			/* Argument strings. */
{
    ...
    enum {BINARY, HEX, DECIMAL, CHAR, STRING, INS, UNSIGNED} format = HEX;
    ...
    /* Change argv[1] to lower-case letters to simplify the access. */
    for (p=argv[1]; *p!=0; p++) if (*p>='A' && *p<='Z')*p = *p - ('A'-'a');
	...
	if (addrIsValue) {
		...
		value = INDEX_TO_ADDR(value);
	        ...
	        value = readRegister(machPtr, address, 0); /* 0 means: no Warnings */
	    }
	} else {
	...
	if (isReg) {
	    address += 1;
	    /* ASIP PIPELINE : NUM_GPRS statt TOTAL_REGS */ 
	    if (address >= NUM_GPRS) {
		break;
	    }
	} else {
    ...
    char **argv;			/* Argument strings. */
{
	...
	if (isReg) {
	    ...
	    default :
	        /* ASIP PIPELINE : */
	        value[0] = readRegister(machPtr, address, 0); /* 0 means: no Warnings */
		...
		if (format == DOUBLE)
		  /* ASIP PIPELINE : */
		  value[1] = readRegister(machPtr, address+1, 0); /* 0 means: no Warnings */
		break;
	    }
	} else {
	    if (ADDR_TO_INDEX(address) < machPtr->numWords) {
    ...
    char **argv;			/* Argument strings. */
{
	...
	*/
	writeRegister(machPtr, address, value, 1);
    } else {
	if (ADDR_TO_INDEX(address) < machPtr->numWords) {
	    MemWord *wordPtr;
    ...
    char **argv;			/* Argument strings. */
{
	...
	*/
	writeRegister(machPtr, address, value[0], 0);

	if (format == DOUBLE) {
	    /* ASIP PIPELINE :
	    machPtr->regs[address+1] = value[1];
	    */
	  writeRegister(machPtr, address+1, value[1], 0);
	}
    } else {
	if (ADDR_TO_INDEX(address) < machPtr->numWords) {
	    MemWord *wordPtr;
					 ...
					 * a register, 0 for memory location. */
{
    ...
    result = Sym_GetSym(machPtr, (char *) NULL, string, SYM_PSEUDO_OK, addressPtr);
    /* ASIP FLOAT : FRegs kann man nicht mehr lesen.
