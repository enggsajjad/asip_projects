...
#ifndef lint
static char rcsid[] = "$Header: /home/cvs/cvs_repos/dlxsim/dlx/dlxsim/tcl/tclUtil.c,v 1.1.1.2 2004/08/30 09:15:06 sascha Exp $ SPRITE (Berkeley)";
#endif
				 ...
				 * Ignored if string is NULL. */
{
    ...
    /*    printf("Tcl_Return: \"%s\" %i; %i %i %i\n", string, status, TCL_STATIC, TCL_DYNAMIC, TCL_VOLATILE);*/
    if (string == NULL) {
    ...
    if (wasDynamic) {
      /* For any reason this line sometimes crashes with the message:
       * "*** glibc detected *** free(): invalid next size (normal): 0x08099cb8 ***"
       * I have looked at this special case where it crashed and I think, that this
       * free should be allowed. But nevertheless it crashes sometimes.
       * Now, where it is outcommented, it is a memory-leak !! */
//      free(oldResult);
    }
}
