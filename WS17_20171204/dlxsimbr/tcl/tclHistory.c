...
#ifndef lint
static char rcsid[] = "$Header: /home/cvs/cvs_repos/dlxsim/dlx/dlxsim/tcl/tclHistory.c,v 1.1.1.2 2004/08/30 09:15:06 sascha Exp $ SPRITE (Berkeley)";
#endif /* not lint */
    ...
    char **argv;			/* Argument strings. */
{
	...
	eventPtr->command = malloc((unsigned) (strlen(argv[2]) + 1));
	strcpy(eventPtr->command, argv[2]);
	return TCL_OK;
    } else if ((c == 'e') && (strncmp(argv[1], "event", length)) == 0) {
	if (argc > 3) {
	    sprintf(iPtr->result,
		    "too many args:  should be \"%.50s get [event]\"",
		    argv[0]);
	    return TCL_ERROR;
	}
	...
	if (argc != 2) {
	    sprintf(iPtr->result, "wrong # args:  should be \"%.50s info\"",
		    argv[0]);
	    return TCL_ERROR;
	}
	count = iPtr->numEvents;
	if (iPtr->curEventNum <= iPtr->numEvents) {
	    count = iPtr->curEventNum;
	}
	length = 0;
	for (i = 0, index = iPtr->curEvent + 1 + iPtr->numEvents - count;
		i < count; i++, index++) {
	    if (index >= iPtr->numEvents) {
		index -= iPtr->numEvents;
	    }
	    length += 9 + strlen(iPtr->events[index].command);
	}
    ...
    sprintf(iPtr->result, "bad \"%.50s\" option \"%.50s\": must be event, info, nextId, redo, or substitute",
		argv[0], argv[1]);
    return TCL_ERROR;
}

					 ...
					 * any given time. */
{
	...
	iPtr->events[i].command = malloc(20);
	*iPtr->events[i].command = 0;
	iPtr->events[i].bytesAvl = 20;
    }
    iPtr->curEvent = 0;
    iPtr->curEventNum = 0;
    ...
    char *string;		/* Description of event. */
{
    ...
    for (index = iPtr->curEvent - 1; index != iPtr->curEvent; index--) {
	if (index < 0) {
	    index += iPtr->numEvents;
	}
				 ...
				 * a pattern. */
{
	...
	if (first > last) {
	    sprintf(iPtr->result, "bad word selector \"%.50s\"", words);
	    return NULL;
	}
    } else {
	pattern = words;
