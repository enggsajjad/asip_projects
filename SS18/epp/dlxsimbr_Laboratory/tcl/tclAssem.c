 ...
 *	from an input source (the commands may arrive in pieces, e.g.
...
#ifndef lint
static char rcsid[] = "$Header: /home/cvs/cvs_repos/dlxsim/dlx/dlxsim/tcl/tclAssem.c,v 1.1.1.2 2004/08/30 09:15:06 sascha Exp $ SPRITE (Berkeley)";
#endif /* not lint */
 ...
 *	The caller should never again use token again.
 *
 * Results:
 *	None.
 *
...
void
Tcl_DeleteCmdBuf(token)
    Tcl_CmdBuf token;		/* Token for command buffer (return value
				 * from previous call to Tcl_CreateCmdBuf). */
{
    register CmdBuf *cbPtr = (CmdBuf *) token;
...
char *
Tcl_AssembleCmd(token, string)
    Tcl_CmdBuf token;		/* Token for a command buffer previously
