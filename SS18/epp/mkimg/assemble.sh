...
GREP='/bin/grep -v'
  ...
  cd ${COMPILER_TEMP_DIR}
##/.comm/s/^/;/; 
  ${SED} '/.glob/s/^/;/; /.align/s/^/;/; /.type/s/^/;/; /.size/s/^/;/; /.ident/s/^/;/; /.zero/s/^/;/; s/.long/.data.32/; /%hi(/ s/)/\/0x10000)/; /%lo(/ s/)/\%0x10000)/; /%hi(/ s/%hi(/(/;/%lo(/ s/%lo(/(/; /^.data/ s/.data/\t.section\t.data\n\t.addressing\t.Word/; s/.rodata.*/.data/; /^.text/ s/.text/\t.section\t.text\n\t.addressing\t.Word/' ${APPLICATION_NAME}.s  > ${APPLICATION_NAME}.s1 || {
      echo
      echo "ERROR: ABORTING \"assemble.sh\", BECAUSE \".S1\" RETURNED AN ERROR !!"
      echo
      exit 1
  }

  ${MKIMG_DIR}/createComm.pl ${APPLICATION_NAME}.s1 > ${APPLICATION_NAME}.s2 || {
      echo
      echo "ERROR: ABORTING \"assemble.sh\", BECAUSE \"createComm.pl\" RETURNED AN ERROR !!"
      echo
      exit 1
  }


  ${MKIMG_DIR}/createDLXsim.pl ${APPLICATION_NAME}.s2 > ${APPLICATION_NAME}.s3 || {
      echo
  ...
  ${MKIMG_DIR}/createString.pl ${APPLICATION_NAME}.s3 > ${APPLICATION_NAME}.s4 || {
      echo
  ...
  ${SED} s/REPLACE/${STACK_START}/ $MKIMG_DIR/startup_dlx2.s > startup_dlx2.s || {
      echo
      echo "ERROR: ABORTING \"assemble.sh\", BECAUSE \"startup_dlx2.s\" RETURNED AN ERROR !!"
      echo
      exit 1
  }

  ${CAT} startup_dlx2.s ${APPLICATION_NAME}.s4 > ${APPLICATION_NAME}.dlxsim || {
      echo
      echo "ERROR: ABORTING \"assemble.sh\", BECAUSE \".dlxsim\" RETURNED AN ERROR !!"
      echo
      exit 1
  }
  ...
  ${CP} ${MKIMG_DIR}/startup.s ${MKIMG_DIR}/handler.s ${MKIMG_DIR}/browtb.x ./. || {
      echo
      echo "ERROR: ABORTING \"assemble.sh\", BECAUSE THE COPYING AN ERROR !!"
      echo
      exit 1
  }
  ...
  ${COMPILER_DIR}/${ASSEMBLER_NAME} handler.s -o handler.o || {
      echo
      echo "ERROR: ABORTING \"assemble.sh\", BECAUSE THE ASSEMBLER RETURNED AN ERROR !!"
      echo
      exit 1
  }
  ...
  ${COMPILER_DIR}/${LINKER_NAME} -o ${APPLICATION_NAME} -T browtb.x *.o ||{
      echo
      echo "ERROR: ABORTING \"assemble.sh\", BECAUSE THE LINKER RETURNED AN ERROR !!"
      echo
      exit 1
  }
  ...
  ${MKIMG_DIR}/gccout2img ${APPLICATION_NAME} ||{
      echo
      echo "ERROR: ABORTING \"assemble.sh\", BECAUSE THE GCCOUT2IMG RETURNED AN ERROR !!"
      echo
      exit 1
  }
  ...
  ${CP} TestData.IM TestData.DM ${MODELSIM_DIR} || {
      echo
      echo "ERROR: ABORTING \"assemble.sh\", BECAUSE THE TESTDATA AN ERROR !!"
      echo
      exit 1
  }
