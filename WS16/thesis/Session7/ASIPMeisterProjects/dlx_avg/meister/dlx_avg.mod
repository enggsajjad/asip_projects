...
"Operand"{"name"{"rs0"},width{"25","21"}},
"Operand"{"name"{"rs1"},width{"20","16"}},
"Operand"{"name"{"rdMin"},width{"15","11"}},
"Operand"{"name"{"rdMax"},width{"10","6"}},
...
},"swap"{type{"R_RA"},"OP-code"{"binary"{"000000"},width{"31","26"}},
"Operand"{"name"{"rs"},width{"25","21"}},
"OP-code"{"binary"{"00000"},width{"20","16"}},
"Operand"{"name"{"rd"},width{"15","11"}},
"OP-code"{"binary"{"00000010001"},width{"10","0"}}
},"minmax"{type{"R_R2"},"OP-code"{"binary"{"000000"},width{"31","26"}},
"Operand"{"name"{"rs0"},width{"25","21"}},
"Operand"{"name"{"rs1"},width{"20","16"}},
"Operand"{"name"{"rdMin"},width{"15","11"}},
"Operand"{"name"{"rdMax"},width{"10","6"}},
"OP-code"{"binary"{"011111"},width{"5","0"}}
}}},
...
,"swap"{Mode{"user_mode"},Behavior{"rd = rs;"},Assert{""},Comment{""},Flags{""},Cycles{"1"}}
,"minmax"{Mode{"user_mode"},Behavior{"if (rs0 < rs1) {
rdMin = rs0;
rdMax = rs1;
}
else {
...
,"swap"{variable{"wire [31:0] source;
wire [31:0] dest;"},clk(1){"wire [31:0] inst;
wire [31:0] current_pc;
current_pc = PC.read();
inst = IMAU.read(current_pc);
null = IR.write(inst);
null = PC.inc();"},
clk(2){"source = GPR.read0(rs);"},
...
clk(4){""},
clk(5){"null = GPR.write1(rd, dest);"}
}
,"minmax"{variable{"wire [31:0] source0;
wire [31:0] source1;
wire [31:0] min;
wire [31:0] max;
wire [3:0] flag;"},clk(1){"wire [31:0] inst;
