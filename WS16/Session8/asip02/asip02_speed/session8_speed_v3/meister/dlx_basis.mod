...
},"clamp"{type{"R_R"},"OP-code"{"binary"{"000000"},width{"31","26"}},
"Operand"{"name"{"rs0"},width{"25","21"}},
"Operand"{"name"{"rs1"},width{"20","16"}},
"Operand"{"name"{"rd"},width{"15","11"}},
"OP-code"{"binary"{"00001000000"},width{"10","0"}}
},"delt"{type{"R_R_R"},"OP-code"{"binary"{"111111"},width{"31","26"}},
"Operand"{"name"{"rs0"},width{"25","21"}},
"Operand"{"name"{"rs1"},width{"20","16"}},
"Operand"{"name"{"rs2"},width{"15","11"}},
"Operand"{"name"{"rd"},width{"10","6"}},
"OP-code"{"binary"{"000000"},width{"5","0"}}
},"sign"{type{"R_R_R"},"OP-code"{"binary"{"111110"},width{"31","26"}},
"Operand"{"name"{"rs0"},width{"25","21"}},
"Operand"{"name"{"rs1"},width{"20","16"}},
"Operand"{"name"{"rs2"},width{"15","11"}},
"Operand"{"name"{"rd"},width{"10","6"}},
...
Operation{NO_VLIW{"add"{Mode{"user_mode"},Behavior{"rd = rd;"},Assert{""},Comment{""},Flags{""},Cycles{"1"}}
...
,"clamp"{Mode{"user_mode"},Behavior{"rd = rd;"},Assert{""},Comment{""},Flags{""},Cycles{"1"}}
,"delt"{Mode{"user_mode"},Behavior{"rd = rd;"},Assert{""},Comment{""},Flags{""},Cycles{"1"}}
,"sign"{Mode{"user_mode"},Behavior{"rd = rd;"},Assert{""},Comment{""},Flags{""},Cycles{"1"}}
}},
...
,"clamp"{variable{"wire [3:0] flag_gt;
wire [31:0] current_pc;
wire [31:0] source0;
wire [31:0] source1;
wire [31:0] source2;"},clk(1){"wire [31:0]  inst;

current_pc = PC.read();
inst = IMAU.read(current_pc);
null = IR.write(inst);
null = PC.inc();"},
clk(2){"source0 = GPR.read0(rd);
source1 = GPR.read1(rs0);
source2 = GPR.read2(rs1);"},
clk(3){"flag_gt = ALU0.cmp(source0, source2);"},
clk(4){""},
clk(5){"wire [2:0] tmp_flag;
wire cond1;
wire cond2;
...
wire lt_sel0;
wire lt_sel1;
wire [31:0] lower_result0;
wire [31:0] lower_result1;

wire lg;
wire nlg;

tmp_flag = flag_gt[2:0];
cond1 = tmp_flag == \"000\";
cond2 = flag_gt == \"0011\";
gt = cond1 | cond2;
...
lower = source1[0];
not_lower = lower == '0';
tmp_flag2 = source0[31:15];
target_msb = source0[31];
lt_tmp2 = tmp_flag2 != \"11111111111111111\";
lt_tmp = target_msb == '1';
lt_res2 = lower & lt_tmp2;
lt_res3 = lt_res2 & lt_tmp;
lt_res = not_lower & lt_tmp;
lt = lt_res3 | lt_res;
lt_sel0 = lt & not_lower;
lt_sel1 = lt & lower;
lower_result0 = \"00000000000000000000000000000000\";
lower_result1 = \"11111111111111111000000000000000\";
lg = lt | gt;
nlg = lg == '0';

null = [lt_sel0] GPR.write0(rd, lower_result0);
null = [lt_sel1] GPR.write0(rd, lower_result1);
null = [gt] GPR.write0(rd, source2);
null = [nlg] GPR.write0(rd, source0);"}
}
,"delt"{variable{"wire [31:0] buffer;
wire [31:0] indata_next;
wire bufferstep;
wire no_bufferstep;
wire [31:0] one;
wire [31:0] zero;
...
clk(2){"source0 = GPR.read0(rs0);
source1 = GPR.read1(rs1);
source2 = GPR.read2(rs2);"},
clk(3){"wire [3:0] flag;

one = \"00000000000000000000000000000001\";
<indata_next, flag> = ALU0.addu(source1, one);"},
clk(4){"wire addr_err;

bufferstep = source2[0];
no_bufferstep = bufferstep == '0';
<buffer, addr_err> = [no_bufferstep] DMAU.lbu(source1);"},
clk(5){"wire [3:0] tmp_buffer;
wire [3:0] tmp_no_buffer;

wire [31:0] delta_buffer;
wire [31:0] delta_no_buffer;
wire [27:0] zero28;

zero28 = \"0000000000000000000000000000\";
zero = \"00000000000000000000000000000000\";
tmp_buffer = source0[3:0];
delta_buffer = <zero28, tmp_buffer>;
tmp_no_buffer = buffer[7:4];
delta_no_buffer = <zero28, tmp_no_buffer>;

null = [bufferstep] GPR.write0(rd, delta_buffer);
null = [no_bufferstep] GPR.write0(rd, delta_no_buffer);
null = GPR.write1(rs0, buffer);
null = [bufferstep] GPR.write2(rs2, zero);
null = [no_bufferstep] GPR.write2(rs2, one);
null = [no_bufferstep] GPR.write3(rs1, indata_next);"}
}
,"sign"{variable{"wire [31:0] result;
wire [31:0] nresult;
wire sign;
wire nsign;
wire [31:0] current_pc;
...
sign = source2[3];
nsign = sign == '0';
<result, flag> = [sign] ALU0.sub(source0, source1);
<nresult, nflag> = [nsign] ALU0.add(source0, source1);"},
clk(4){""},
clk(5){"null = [sign] GPR.write0(rd, result);
null = [nsign] GPR.write0(rd, nresult);"}
}
}}
