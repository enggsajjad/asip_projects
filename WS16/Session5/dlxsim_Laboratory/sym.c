...
#ifndef lint
//static char rcsid[] = "$Header: /home/cvs/cvs_repos/dlxsim/dlx/dlxsim/sym.c,v 1.1.1.2 2004/08/30 09:15:05 sascha Exp $ SPRITE (Berkeley)";
#endif /* not lint */
				 ...
				 * SYM_REGISTER, SYM_GLOBAL, SYM_NO_ADDR. */
{
    ...
    symPtr->fileName = (char *) calloc(1, (unsigned) (strlen(fileName) + 1));
				 ...
				 * gets stored here. */
{
	...
	*/
	...
	/* ASIP REGISTER : AEnderung, damit sowohl "r", als auch "%r" als Register-Prefix akzeptiert werden. NEU: Auch "GPR" und "%GPR" */
	if ( (*name=='r') || (strncmp(name, "%r", 2)==0) || (strncmp(name, "%gpr", 4)==0) || (strncmp(name, "gpr", 3)==0) )  {
            if (*name=='%') name++;
	    if ( strncmp(name, "gpr", 3)==0 ) name += 2;  /* +2 instead of +3 (gpr=>3 letters), because the later commands use "name+1" to be able to deal with inputs, like %pc, ...*/
	    num = strtoul(name+1, &end, 10);
	    if ((end != (name+1)) && (*end == 0) && (num <= 31) && (num >= 0)) {
		gotReg:
		if (num >= 32) {
 ...
 *----------------------------------------------------------------------
 */
char* lastSymbol=0;  // The last symbol is used for detecting the "lhi rd, label" instructions for jump tables.
static int
Lex(infoPtr)
    register ExprInfo *infoPtr;		/* Describes the state of the parse. */
{
    ...
    c = *p;
    while (isspace(c) || c=='$' ) { /* ASIP : Das Dollar-Zeichen wird einfach uebersprungen, um die pas-konformen immediates richtig einlesen zu koennen. */
	p++;  c = *p;
    }
    infoPtr->expr = p;
    if (!isascii(c)) {
	...
	for (p++; isalnum(*p) || (*p == '_') /* ASIP CONSTANT : || (*p == '$')*/; p++) {
	...
	*p = 0;
	...
	if (lastSymbol!=0) free(lastSymbol);
	if (infoPtr->tokenChars==0) {
	  lastSymbol=0;
	} else {
	  lastSymbol = (char*)malloc((strlen(infoPtr->tokenChars)+1)*sizeof(char));
	  tempPointer = infoPtr->tokenChars;
	  while (*tempPointer==' ' || *tempPointer=='\t') tempPointer++;
	  tempPointer2 = lastSymbol;
	  while (*tempPointer!=' ' && *tempPointer!='\t' && *tempPointer!='\0' && *tempPointer!='\n') {
				 ...
				 * is returned. */
{
	...
	sprintf(string, "%.50s+0x%.2x (0x%.4x)", closestName, closestDistance, address);
    }
    return string;
}
