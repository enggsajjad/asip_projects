...
asm int clamp_func (int a, int b, int c)
{
	@[.barrier]
...
asm int predict_func (int a, int b, int c)
{
	@[.barrier]
	; This comment is written to the created assembly file
	;nop
...
asm int stepsft_func (int a, int b, int c)
{
	@[.barrier]
	; This comment is written to the created assembly file
	;nop
...
//	t_print("\r\n");
	...
	*/
		//vpdiff = step >> 3;
		vpdiff = stepsft_func(vpdiff, step, 3);
	if ( delta & 4 ) 
		//vpdiff += step;
		vpdiff = stepsft_func(vpdiff, step, 0);
	if ( delta & 2 ) 
		//vpdiff += step>>1;
		vpdiff = stepsft_func(vpdiff, step, 1);
	if ( delta & 1 ) 
		//vpdiff += step>>2;
		vpdiff = stepsft_func(vpdiff, step, 2);

	/*if ( sign )
	  valpred -= vpdiff;
	else
	  valpred += vpdiff;*/
	valpred=predict_func(valpred,vpdiff,sign);
	//t_print("\r\n sign=");
	//t_printHex(sign, 4);
	//t_print("\r\n");

	//t_print("\r\n valpred=");
//	t_printHex(valpred, 4);//
//	t_print("\r\n");
	...
	if ( valpred > 32767 )
	  ...
	  valpred = -32768;*/
	valpred=clamp_func(valpred,-32768,32767);
//	t_print("\r\n valpred=");
//	t_printHex(valpred, 4);
//	t_print("\r\n");
...
#define signedUnsignedModification 0x0
#if PRINT_ARRAY
	t_print(separator);
