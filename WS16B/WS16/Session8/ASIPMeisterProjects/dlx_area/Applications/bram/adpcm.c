...
asm int clamp_low_func (int a, int b)
{
	@[.barrier]
...
asm int clamp_high_func (int a, int b)
{
	@[.barrier]
	; This comment is written to the created assembly file
	;nop
...
asm int predict_add_func (int a, int b, int c)
{
	@[.barrier]
	; This comment is written to the created assembly file
	;nop
...
asm int predict_sub_func (int a, int b, int c)
{
	@[.barrier]
	; This comment is written to the created assembly file
	;nop
...
int adpcm_decoder(unsigned char* indata, int len) {
	...
	//if ( index < 0 ) index = 0;
	index=clamp_low_func(index,0);
	//if ( index > 88 ) index = 88;
	index=clamp_high_func(index,88);
	...
	*/
	...
	if ( sign )
	  //valpred -= vpdiff;
	  valpred=predict_sub_func(valpred,vpdiff,sign);
	else
	  //valpred += vpdiff;
	  valpred=predict_add_func(valpred,vpdiff,sign);
	...
	//  valpred = -32768;
	valpred=clamp_high_func(valpred,32767);
	valpred=clamp_low_func(valpred,-32768);
